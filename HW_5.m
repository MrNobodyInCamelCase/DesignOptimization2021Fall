...
% objective:
df = @(x) [2*x(1) , 2*x(2)-6]; % gradient

g = @(x) [x(2)^2-2*x(1); (x(2)-1)^2+5*x(1)-15]; % constraint
dg = @(x) [-2 2*x(2); 5 2*x(2)-2]; % grad of constraint
...
%% Report
%report(solution,f,g);
% ALL VALUES PRINTED IN THE COMMAND WINDOW OR CAN BE RETRIEVED FROM WORKSPACE
val_x = solution.x(:,end) % generates the values of x a an array
val_g = g(solution.x(:,end)) % generates the values of g a an array
val_f = f(solution.x(:,end)) % final solution
